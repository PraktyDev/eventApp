...
const AddDjPage = () => {
  ...
  const handleSubmit = async (e) =>{
    e.preventDefault();
    try {
      const response = await fetch("/admin/add-dj/api",{
        method: 'POST',
    ...
    <section className="p-4 mt-20 h-full max-w-md mx-auto flex flex-col border border-blue-200 justify-between">
      <form onSubmit={handleSubmit} className="flex flex-col gap-12">
        ...
        <div className="flex flex-col gap-6 items-center">
          <button className="bg-blue-800 hover:bg-blue-700 text-white w-full rounded-md py-2 ">Add DJ</button>
        </div>
      </form>
    </section>
  )
