...
const fetchCatering = async () => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/add-catering/api`);
  if (!response.ok) {
...
const CateringServicePage = async () => {
  const catering = await fetchCatering();
  ...
  return (
    <section className="flex flex-col h-screen w-full p-4 gap-3">
      ...
      <div className="grid grid-cols-2 tablet:grid-cols-3 laptop:grid-cols-4 gap-4 w-full ">
        {catering.map((item) => (
          <div key={item._id} className="flex flex-col">
            <Link href={`/services/catering/${item._id}`}>
              <Image
                src={item.image}
                width={100}
                height={100}
