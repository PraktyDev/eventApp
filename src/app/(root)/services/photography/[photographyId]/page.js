...
const PhotographyIdPage = async ({ params }) => {   
    const { photographyId } = params

    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/add-photography/api/${params.photographyId}`);
    ...
    return (
        <section className="flex flex-col h-svh w-full p-4 gap-3 justify-between">
            ...
            <div className="flex gap-2 items-center">
                <Link href="/services/photography"><FaArrowLeft /></Link>
                <p>{data.name}</p>
            </div>
...
export default PhotographyIdPage;
