...
export default function UpcomEvent() {
  ...
  return (
    ...
    <div className='flex basis-1/4 flex-col gap-4 justify-center items-center'>
      ...
      <Link href='/events' className='p-1 text-center w-[100px] rounded-sm text-white bg-orange-600 hover:bg-orange-500'>See More</Link>
    </div>
    ...
    {event.map(item =>(      
        ...
        <Link href={`events/${item._id}`}>
            <Image width={16} height={9} alt="card" src={item.image} className="object-cover rounded-lg bg-slate-600 w-40 tablet:w-40 laptop:w-72 h-32 tablet:h-40" />
